...
// Change: Updated data structure to hold all required values
export interface SimulationDataPoint {
  time: number;
  stroke: number;         // For Displacement
  velocity: number;       // For Velocity
  pressure_rod: number;   // For Pressure at rod end
  pressure_cap: number;   // For Pressure at cap end
  flow: number;           // For Flow rate of pump
  motorPower: number;
    ...
    try {
      ...
      phases.forEach(phase => {
        ...
        for (let i = 0; i <= dataPointsPerPhase; i++) {
          ...
          if (phase.name === 'fastUp') { strokeAtTime -= phaseParams.speed * timeInPhase; } 
          else { strokeAtTime += phaseParams.speed * timeInPhase; }
          ...
          simulationDataPoints.push({
            time, flow: phase.flow - flowVariation, stroke: Math.max(0, strokeAtTime),
            pressure_cap: phase.pressure - pressureVariation, // Map to cap pressure
            pressure_rod: phase.pressure * 0.2, // Approximate rod pressure
            velocity: phaseParams.speed / 1000,
            motorPower: phase.motorPower, actuatorPower: phase.actuatorPower,
            phase: phase.name.replace(/([A-Z])/g, ' $1').replace(/^./, (str) => str.toUpperCase()),
            pumpInputPower: phase.pressure * phase.flow / 600,
            actualMotorInputPower: phase.motorPower, actuatorOutputPower: phase.actuatorPower, idealMotorInputPower: phase.idealPower
          });
        }
        if (phase.name === 'fastUp') { currentStroke -= phaseParams.stroke; } 
        else { currentStroke += phaseParams.stroke; }
